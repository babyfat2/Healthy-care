...
export class Aaa1741148310245 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`appointments\` (\`id\` int NOT NULL AUTO_INCREMENT, \`pateint_id\` int NOT NULL, \`hospital_id\` int NOT NULL, \`appointment_time\` datetime NOT NULL, \`status\` enum ('PENDING', 'COMFIRMED', 'COMPLETED', 'CANCELLED') NOT NULL DEFAULT 'PENDING', \`userId\` int NULL, \`hospitalId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(256) NOT NULL, \`phone\` varchar(11) NULL, \`password\` varchar(256) NOT NULL, \`full_name\` varchar(256) NULL, \`address\` varchar(256) NULL, \`role\` enum ('PATEINT', 'DOCTOR', 'HOSPITAL') NOT NULL DEFAULT 'PATEINT', \`doctor_id\` int NULL, \`hospital_id\` int NULL, UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), UNIQUE INDEX \`REL_6ea478e0c37ff2f69e32cfb94c\` (\`doctor_id\`), UNIQUE INDEX \`REL_6da026cf705d995e70c1b1c106\` (\`hospital_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
